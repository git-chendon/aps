...
public class Selecting {
    ...
    public List<int[][]> selectNextGeneration(List<int[][]> P, List<int[][]> C1, List<int[][]> C2,
                                              List<Order> orderSorted, List<Job> originJobList, int machineNum, int fitFlag) {
        List<int[][]> result = new ArrayList<>();

        //计算各种群的适应度，并排序输出
        int[][] sortFitP = sortByFit(P, orderSorted, originJobList, machineNum, fitFlag);
        int[][] sortFitC1 = sortByFit(C1, orderSorted, originJobList, machineNum, fitFlag);
        int[][] sortFitC2 = sortByFit(C2, orderSorted, originJobList, machineNum, fitFlag);
        ...
        result = sortChromGroup(result, orderSorted, originJobList, machineNum, fitFlag);  //排序输出，保证最优结果
        return result;
    }
    ...
    private int[][] sortByFit(List<int[][]> chromGroup, List<Order> orderSorted,
        ...
        int[] fit = fitness.fitness(chromGroup, orderSorted, originJobList, machineNum, fitFlag);
    ...
    public List<int[][]> sortChromGroup(List<int[][]> chromGroup, List<Order> orderSorted,
        ...
        Fitness fitness = new Fitness();
        List<int[][]> result = new ArrayList<>();
        int[] fit = fitness.fitness(chromGroup, orderSorted, originJobList, machineNum, fitFlag); //找到每个个体适应度
