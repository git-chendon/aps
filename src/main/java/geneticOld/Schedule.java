...
public class Schedule {
    ...
    //输出各工序开始时间，结束时间；（机器负载）
    //排程群组
    public List<List<Job>> scheduleGroup(List<List<Job>> jobArrayGroup, int[][] workDurationTime) {
        List<List<Job>> jobShiftingGroup = new ArrayList<List<Job>>();
        for (List<Job> tempJobArray : jobArrayGroup) {      //遍历群组
            List<Job> tempJobList;
            tempJobList = scheduleSingle(tempJobArray, workDurationTime);
            jobShiftingGroup.add(tempJobList);
        }
        return jobShiftingGroup;
    }
    //排程一组
    private List<Job> scheduleSingle(List<Job> JobArray, int[][] workDurationTime) {
